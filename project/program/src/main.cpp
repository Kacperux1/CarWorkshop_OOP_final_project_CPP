...
void createNewClient(){
    ...
    while(flag)
    {
        ...
        cout<<"Client with tath id already exist! Enter another identificator."<<endl;
    }
...
void createNewCar(){
    ...
    while(flag)
    {
        ...
        cout<<"Car with this plate numebr already exist! Enter another number."<<endl;
    }
...
void getOrderCost(){
    int id;
    cout<<"Enter id of the order you want to get cost of: ";
    cin>>id;
    cout<<"Current cost of this order is: "<<orderMgr->findOrder(id)->getCost()<<endl;
...
void createNewOrder(){
    ...
    while(flag)
    {
        ...
        cout<<"Order with this id already exists! Enter different id."<<endl;
    }
    ...
    orderMgr->changeRepairType(orderMgr->createOrder(id, daysForRepair, (carMgr->findCar(carId))->getClient(), carMgr->findCar(carId)), type);
}
...
int main()
{
    ...
    while(flag)
    {
        ...
        <<endl<<"7. Find certain car, client or order by id/plate number"<<endl<<"8. archive the repair order"<<endl<<"9. Get cost of the order"<<endl<<
        "10. Return an order to the client"<<endl<<"11. Exit program."<<endl;
        cin>>choice;
        switch(choice) {
            case 1:
                createNewClient();
            ...
            case 5:
                ...
                switch (choice2) {
                    case 1:
                        ...
                        cout<<clientMgr->getAllClientInfo()<<endl;
                        break;
                    case 2:
                        if (carMgr->count() == 0)
                            cout << "There are currently no cars." << endl;
                        cout<<carMgr->getAllCarInfo()<<endl;
                        break;
                    case 3:
                        if (orderMgr->count() == 0)
                            cout << "There are currently no orders." << endl;
                        cout<<orderMgr->getAllOrderInfo()<<endl;
                        break;
                    default:
                ...
                break;
            case 6:
                cout<< "You want to save info to file about clients, cars or orders? Enter the number subsequently from 1 to 3:";
                cin >> choice2;
                switch (choice2) {
                    case 1:
